...
/*
**     虚拟机有两种不同的实现方式: 基于栈的虚拟机和基于寄存器的虚拟机. Lua是已
** 知的第一个使用基于寄存器的虚拟机. 两者的区别如下:
**     在基于栈的虚拟机中, 字节码的操作数是从栈顶上弹出(pop), 在执行完操作后再
** 压入(push)栈顶. 以加法为例:
** pop 2
** pop 2
** add 2, 1, result
** push result
**     可见, 执行一条加法指令需要4条字节码. 其中前几条指令用于准备数据, 将数据
** 压栈, 这是这种设计的缺点. 但优点是, 指令中不需要关系操作数的地址, 在执行操作
** 之前已经将操作数准备在栈顶上了.
**     与基于栈的虚拟机不同, 在基于寄存器的指令中, 操作数是放在"CPU的寄存器"中
** (因为并不是物理意义上的寄存器). 因此, 同样的操作不在需要PUSH, POP指令, 取而
** 代之的是在字节码中带上具体操作数所在的寄存器地址. 同样, 以加法为例:
** ADD R1, R2, R3 #寄存器R1与R2的相加结果保存在寄存器R3中.
**     可见, 对比基于栈的寄存器, 只需要一条指令就可以完成加法操作. 但缺点是此时
** 程序需要关注操作数所在的位置.
**     lua 使用的是基于寄存器的虚拟机的实现方式, 其中很大的原因是它的设计目标之
** 一就是尽可能高效.
*/
...
/*
** 当函数引用的常量较少时, Lua允许让具体的指令直接访问常量表, 而不需要将常量读取到寄存
** 器(B or C). 为了区分使用一位来表示是寄存器, 还是常量编号
...
#define MAXINDEXRK	(BITRK - 1)  // 所能表示的最大的编号(猜测)
#endif
...
typedef enum {
    ...
    ** 大部分的指令都是直接对寄存器进行操作, 而不需要对数据进行压栈和弹栈, 所以需要OP_MOVE
    ** 指令的地方并不多. 最直接的使用之处就是将一个local变量复制给另一个local变量时
    */
    OP_MOVE,/*	A B	R(A) := R(B)					*/
    /*
    ...
    */
    ...
    /*
    ** 将B为索引的upvalue当作一个table, 并将C做为索引的寄存器或者常量当作key获取的值放入
    ** 寄存器A
    */
    ...
    */
    ...
    /*
    ** 将A为索引的upvalue当作一个table, 将C寄存器或者常量的值以B寄存器或常量为key, 存入
    ** table
    */
    ...
    ** B和C使用"floating point byte"的方法来表示成(eeeeexxx)的二进制形式, 其实际值
    ** 为(1xxx) * 2^(eeeee-1)
    */
    OP_NEWTABLE,/*	A B C	R(A) := {} (size = B,C)				*/
    /*
    ** 做好调用成员函数之前的准备, 其中待调用模块赋值到R(A+l)中, 而待调用的成员函数存放在
    ** R(A)中,
    ** 待调用的模块存放在R(B)中, 待调用的函数名存放在RK(C)中
    ** 专门为":"运算符准备的指令. 从寄存器B表示的table中, 获取出C作为key的closure, 存入
    ** 寄存器A中, 然后将table本身存入到寄存器A＋1中, 为接下来调用这个closure做准备
    */
    ...
    ** 如果A>0, 表示需要关闭所有从寄存器A+1开始的所有local变量. 实际执行的关闭操作只
    ** 对upvalue有效.
    */
    OP_JMP,/*	A sBx	pc+=sBx; if (A) close all upvalues >= R(A - 1)	*/
    /*
    ** 关系指令对RK(B)和RK(C)进行比较, 然后将比较结果与A指定的boolean值进行比较, 来决定最
    ** 终的boolean值.
    ...
    ** B－1表示参数个数. 如果参数列表的最后一个表达式是变长的, 则B会设置为0, 表示使用A＋1
    ** 到当前栈顶作为参数.
    ** 函数调用的返回值会按顺序存放在从寄存器A开始的C-1个寄存器中. 如果C为0, 表示返回值的
    ** 个数由函数决定.
    */
    ...
    */
    ...
    OP_FORLOOP,/*	A sBx	R(A)+=R(A+2);
                ...
                ** 参数A: OP_NEWTABLE 指令中创建好的表所在的寄存器, 它后面紧跟着待写入的数据
                ** 参数B: 待写入数据的数量
                ** 参数C: FPF(LFIELDS_PER_FLUSH)索引, 即每次写入最多是LFIELDS_PER_FLUSH
                */
